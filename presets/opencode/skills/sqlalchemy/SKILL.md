...
All models inherit from `Base`. All persistent entities include `TimestampMixin`.
...
Always use `back_populates` (explicit) — never `backref` (implicit, hard to trace).
`lazy="raise"` catches N+1 at runtime instead of silently degrading performance.
...
**Rule**: Never call `session.commit()` in services — commit belongs in `get_db`.
Use `await db.flush()` to flush to DB within a transaction without committing.

## Transactions

The `get_db` dependency owns the transaction. For nested operations requiring
an explicit savepoint:
...
Use `begin_nested()` for savepoints within an existing transaction.
Never call `db.begin()` inside a service — the session is already in a transaction.
