...
/**
 * Validates that generated Agy files follow expected patterns:
 * - Paths must start with .agents/
 * - At least one skill file must exist
 * - Agent files must follow .agents/agents/council-*.md pattern
 */
function validateAgyFiles(files: readonly GeneratedFile[]): string[] {
  const errors: string[] = [];

  for (const file of files) {
    if (!file.path.startsWith('.agents/')) {
      errors.push(`Agy file path must start with .agents/: ${file.path}`);
    }
    if (typeof file.content !== 'string' || file.content.length === 0) {
      errors.push(`Agy file content must be a non-empty string: ${file.path}`);
    }
  }

  const hasSkill = files.some((f) => f.path.includes('/skills/'));
  if (!hasSkill && files.length > 0) {
    errors.push('Agy output must include at least one skill file (.agents/skills/)');
  }

  const hasAgent = files.some((f) => f.path.includes('/agents/'));
  if (!hasAgent && files.length > 0) {
    errors.push('Agy output must include at least one agent file (.agents/agents/)');
  }
...
export class AgyAgentContractRenderer implements AgentContractRenderer {
  ...
  render(contract: AgentContract): AgentContractRenderResult {
    ...
    const files = generateAgyFiles(contract.council);

    // Validate generated output
    const validationErrors = validateAgyFiles(files);
