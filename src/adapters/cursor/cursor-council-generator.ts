...
/**
 * Generate Cursor council files.
 *
 * Cursor shares Claude's command/skill/agent format (Markdown, `description:`
 * frontmatter, `/cc:` colon invocation) — this mirrors
 * claude-council-generator.ts under `.cursor/` instead of `.claude/`.
 */
export function generateCursorFiles(spec: CouncilSpec): GeneratedFile[] {
  const files: GeneratedFile[] = [];

  files.push({
    path: '.cursor/skills/council/SKILL.md',
  ...
  for (const agent of spec.agents) {
    files.push({
      path: `.cursor/agents/council-${agent.id}.md`,
  ...
  files.push({
    path: '.cursor/commands/cc/council.md',
