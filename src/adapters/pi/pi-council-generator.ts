...
/**
 * Generate Pi council files.
 *
 * Pi discovers skills the same way agy does (`.agents/skills/`, per its own
 * docs), so the skill and per-role reference files are shared with agy under
 * `.agents/`. The workflow itself is a Pi prompt template
 * (`.pi/prompts/cc-council.md`) — Pi's discovery is flat (`.pi/prompts/*.md`,
 * filename = command), and it supports `$ARGUMENTS` directly, so no
 * placeholder translation is needed. Pi has no Task tool or subagents, so
 * the workflow uses the same "invoke `X` with `<context>`" phrasing agy and
 * opencode already use for the identical reason.
 */
export function generatePiFiles(spec: CouncilSpec): GeneratedFile[] {
...
The council will evaluate the diff against the roster's axes (Architecture, Security, Product, Delivery, DataOps, Devil).
