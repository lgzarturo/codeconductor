...
describe('v0.2.0 roadmap completion', () => {
  ...
  test('detectProject assigns deterministic confidence levels', async () => {
    ...
    const springProfile = await detectProject(join(PROJECT_ROOT, 'test', 'fixtures', 'spring-project'))
  ...
  test('prompt changelog discipline requires CHANGELOG.md for prompt contract changes', () => {
    ...
    const valid = validatePromptChangelog(['presets/opencode/commands/cc-feature.md', 'CHANGELOG.md'])
  ...
  test('install preset --dry-run exposes preset resolution in json', async () => {
    ...
    const result = await runCli(['install', 'preset', '--target=codex', '--dry-run', '--output=json'])
    expect(result.exitCode).toBe(0)
  ...
  test('doctor reports structured target security compatibility gaps', async () => {
    ...
    expect(json.securityCompatibility.find((target: { target: string }) => target.target === 'codex').unsupportedRules).toContain('runtime.network')
    expect(json.checks.find((check: { name: string }) => check.name === 'security-codex').status).toBe('warn')
  })
  ...
  test('install preset copies API contract and database migration workflows', async () => {
    ...
    expect(existsSync(join(PROJECT_ROOT, '.opencode', 'commands', 'cc-api-contract.md'))).toBe(true)
    expect(existsSync(join(PROJECT_ROOT, '.opencode', 'commands', 'cc-db-migration.md'))).toBe(true)
    expect(existsSync(join(PROJECT_ROOT, '.claude', 'commands', 'cc', 'api-contract.md'))).toBe(true)
    expect(existsSync(join(PROJECT_ROOT, '.claude', 'commands', 'cc', 'db-migration.md'))).toBe(true)
  })
  ...
  test('merge-managed manifest updates only managed Codex AGENTS block', async () => {
    ...
    await writeFile(join(PROJECT_ROOT, '.codex', 'AGENTS.md'), [
      '# Local heading',
      '',
      '<!-- CODECONDUCTOR:BEGIN managed -->',
      'old generated block',
  ...
  test('source workflows include bug-fix and refactor discipline', async () => {
    const fix = await readFile(join(PROJECT_ROOT, 'presets', 'opencode', 'commands', 'cc-fix.md'), 'utf-8')
    const refactor = await readFile(join(PROJECT_ROOT, 'presets', 'opencode', 'commands', 'cc-refactor.md'), 'utf-8')
